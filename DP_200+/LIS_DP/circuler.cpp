...
// int lis(vi&arr, int start, int end, int window){
//       vi dp(end-start, 1);
//       int res = INT_MIN;
//       for(int i=start+1; i<end; i++){
//             for(int prev = start; prev<i; i++){
...
int optimised(vi&arr, int start, int end){
      vector<int> ans;
...
int lis_tab(vector<int>&arr){
      int n=arr.size();
      vector<int>cir_arr(2*n);
      for(int i=0;i<2*n;i++){
            cir_arr[i]= arr[i%n];
      }
      ...
      int ans=INT_MIN;
      for(int i=0;i<n;i++){
            ans= max(ans,optimised(cir_arr, i, i+n)); 
      }
      return ans;
}
