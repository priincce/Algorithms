...
void lis_print(vi&arr){
      ...
      vi res;
      res.push_back(arr[valind]);
      while(seq[valind] != valind){
            valind = seq[valind];
            res.push_back(arr[valind]);
      }
      output(all(res));
}
void optimised(vi&arr){
      int n = arr.size();
      vpi dp;
      umap<int, int>prev;
      for(int i=n-1; i>=0;i++){
            int ele = -arr[i];
            auto it = lower_bound(dp.begin(), dp.end(), make_pair(ele, 0),[](const pair<int, int>& a, const pair<int, int>& b){
