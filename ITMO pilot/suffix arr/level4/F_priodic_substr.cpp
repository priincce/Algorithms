...
void countSort(vector<int> &p, vector<int> &c) {
    ...
    vector<int> pos(n);
    pos[0] = 0;
    for (int x = 1; x < n; x++) pos[x] = pos[x - 1] + cnt[x - 1];
    vector<int> p_new(n);
    for (auto x : p) {
        p_new[pos[c[x]]] = x;
        pos[c[x]]++;
...
pair<vector<int>, vector<int>> computeSuffixArray(string s) {
    ...
    vector<int> pos(n), class_(n);
    {
        ...
        class_[pos[0]] = 0;
        for (int i = 1; i < n; i++) class_[pos[i]] = class_[pos[i - 1]] + int(a[i - 1].first != a[i].first);
    }
    int k = 0;
    while ((1 << k) < n && class_[pos[n - 1]] < n - 1) {
        ...
        class_new[pos[0]] = 0;
        for (int i = 1; i < n; i++) {
            pair<int, int> prev = {class_[pos[i - 1]], class_[(pos[i - 1] + (1 << k)) % n]};
...
vector<int> computeLCP(const vector<int> &p, const vector<int> &c, const string &s) {
    int n = p.size();
    int k = 0;
    vector<int> lcp(n - 1);
    for (int i = 0; i < n - 1; i++) {
